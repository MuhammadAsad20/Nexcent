...
function Footer() {
  return (
    <footer className="bg-slate-900 text-white body-font">
      <div className="container px-5 py-24 mx-auto flex md:items-start items-center md:flex-row flex-col">
        <div className="w-64 flex-shrink-0 md:mx-0 mx-auto text-center md:text-left">
          ...
          <div className="flex justify-center md:justify-start mt-4">
            {['facebook', 'twitter', 'instagram', 'linkedin'].map((platform, index) => (
              <a key={index} className="text-gray-500 ml-3 first:ml-0">
            ...
            <div key={index} className="lg:w-1/4 md:w-1/2 w-full px-4">
              ...
              <nav className="list-none mb-10 space-y-2">
                {['First Link', 'Second Link', 'Third Link', 'Fourth Link'].map((link, i) => (
                  <li key={i}>
                    <a className=" hover:text-gray-800 cursor-pointer">{link}</a>
                  </li>
                ))}
              </nav>
            </div>
